...
@section('content')
    <div class="container-fluid">
        <div class="row admin-panel">
            <a href="/adminallposts" class="pull-left btn btn-default">
                << Բոլոր նյութերը</a>
                    ...
                    <div class="col-md-8 col-md-offset-2">
                        <form action="/savepost" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control" name="title" placeholder="Վերնագիր"
                                    value="{{$post->title}}" required>
                            </div>
                            <div class="form-group">
                                <input type="text" class="form-control" name="anons" placeholder="Կարճ նկարագրություն"
                                    value="{{$post->anons}}" required>
                            </div>
                            <div class="form-group">
                                <textarea class="form-control summernote" name="description" rows="15"
                                    placeholder="Մանրամասն նկարագիր" required>{{$post->description}}</textarea>
                                <p class='small text-danger'>Տեքստի մեջ նկար չի կարելի բեռնել, պետք է նախորոք բեռնել
                                ...
                                @foreach($gals as $gal)
                                    @if($gal)
                                        <div class="col-md-3 text-center"
                                            ...
                                            <a href="/deletegal/{{$post->id}}/{{$gal}}" class="btn btn-danger delete"
                                                style="position: absolute;top:0;left:0;z-index: 999;color:white;">X</a>
                            ...
                            <div class="form-group d-flex justify-content-between align-items-center">
                                ...
                                <input type="checkbox" name="norutyun" id="norutyun" value="1"
                                    @if($post->categories->contains('slug', 'norutyun')) checked @endif>
                            </div>

                            <div class="form-group d-flex justify-content-between align-items-center">
                                <label for="harcazruyc" class="mb-0">Հարցազրույց</label>
                                <input type="checkbox" name="harcazruyc" id="harcazruyc" value="1"
                                    @if($post->categories->contains('slug', 'harcazruyc')) checked @endif>
                            </div>
                            <div class="form-group">
